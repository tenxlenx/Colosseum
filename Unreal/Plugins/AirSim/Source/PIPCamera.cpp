...
void APIPCamera::PostInitializeComponents()
{
    ...
    for (unsigned int i = 0; i < cubeTypeCount(); ++i) {
        detections_cube_[i] = NewObject<UDetectionComponentCube>(this);
...
void APIPCamera::enableCaptureComponent(const APIPCamera::ImageType type, bool is_enabled)
{
    if (!ImageCaptureBase::isCubeType(type)) {
        ...
        if (capture != nullptr) {
            UDetectionComponentCube* detection_cube = getDetectionComponentCube(type, false);
            if (is_enabled) {
                //do not make unnecessary calls to Activate() which otherwise causes crash in Unreal
                if (!capture->IsActive() || capture->TextureTarget == nullptr) {
                    capture->TextureTarget = getRenderTargetCube(type, false);
                    capture->Activate();
                    if (detection_cube != nullptr) {
                        detection_cube->texture_target_ = capture->TextureTarget;
                        detection_cube->Activate();
                    }
                }
            }
            else {
                if (capture->IsActive() || capture->TextureTarget != nullptr) {
                    ...
                    capture->TextureTarget = nullptr;
                    if (detection_cube != nullptr) {
                        detection_cube->Deactivate();
                        detection_cube->texture_target_ = nullptr;
                    }
                }
            }
            camera_type_enabled_[Utils::toNumeric(type)] = is_enabled;
...
UDetectionComponentCube* APIPCamera::getDetectionComponentCube(const ImageType type, bool if_active) const
{
    unsigned int image_type = Utils::toNumeric(type);

    if (!if_active || camera_type_enabled_[image_type])
        return detections_cube_[11 - image_type];
    return nullptr;
}
...
void APIPCamera::copyCameraSettingsToAllSceneCapture(UCameraComponent* camera)
{
    ...
    for (int image_type = image_count - 1; image_type >= 0; image_type--) {
        if (image_type < 10) {
            ...
            copyCameraSettingsToSceneCaptureCube(camera_, captures_cube_[0]);
        }

        else {
            copyCameraSettingsToSceneCaptureCube(camera_, captures_cube_[1]);
        }
    }
}
