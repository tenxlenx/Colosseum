...
public:
    struct RenderParams
    {
        ...
        RenderParams(USceneCaptureComponent2D* render_component_val, UTextureRenderTarget2D* render_target_val, bool pixels_as_float_val, bool compress_val)
            : render_component(render_component_val), render_target(render_target_val), pixels_as_float(pixels_as_float_val), compress(compress_val), render_component_cube(nullptr), render_target_cube(nullptr), is_cube(false)
        {
        }
        ...
        RenderParams(USceneCaptureComponentCube* render_component_val, UTextureRenderTargetCube* render_target_val, bool pixels_as_float_val, bool compress_val)
            : render_component(nullptr), render_target(nullptr), pixels_as_float(pixels_as_float_val), compress(compress_val), render_component_cube(render_component_val), render_target_cube(render_target_val), is_cube(true)
        {
        }
    };
    struct RenderResult
